...
from piglot.objective import Composition, StochasticSingleCompositeObjective
try:
...
try:
    ...
    from piglot.optimisers.optimiser import CompositeStochasticOptimiser
except ImportError:
...
class BayesianBoTorchCompositeStochastic(CompositeStochasticOptimiser):
    ...
    def _optimise(
        self,
        objective: StochasticSingleCompositeObjective,
        n_dim: int,
        n_iter: int,
        bound: np.ndarray,
        ...
        init_response, variance = objective(init_shot)
        ...
        dataset.push(init_shot, init_response, variance)
        ...
        for i, (response, variance) in enumerate(init_responses):
            dataset.push(random_points[i], response, variance)
        ...
        for i, (response, variance) in enumerate(test_responses):
            test_dataset.push(test_points[i], response, variance)
        ...
        for i_iter in range(n_iter):
            ...
            losses = [objective.composition(response) for (response, _) in responses]

            # Find best value for this batch and update dataset
            best_idx = np.argmin(losses)
            best_loss, best_params = losses[best_idx], candidates[best_idx, :]
            for i, (response, variance) in enumerate(responses):
                dataset.push(candidates[i, :], response, variance)
