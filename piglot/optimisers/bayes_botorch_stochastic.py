...
try:
    ...
    from piglot.optimisers.optimiser import ScalarStochasticOptimiser
except ImportError:
...
class BayesianBoTorchStochastic(ScalarStochasticOptimiser):

    def __init__(self, n_initial=5, acquisition='ucb', log_space=False,
    ...
    def _optimise(
        self,
        objective: StochasticSingleObjective,
        n_dim: int,
        n_iter: int,
        bound: np.ndarray,
        ...
        value, variance = objective(init_shot)
        dataset.push(init_shot, loss_transformer(value), variance)
        ...
        for i, (loss, variance) in enumerate(init_losses):
            dataset.push(random_points[i], loss_transformer(loss), variance)
        ...
        if self.n_test > 0:
            ...
            for i, (loss, variance) in enumerate(test_losses):
                test_dataset.push(test_points[i], loss_transformer(loss), variance)
        ...
        for i_iter in range(n_iter):
            ...
            results = self._eval_candidates(objective, candidates)
            ...
            for i, (loss, variance) in enumerate(results):
                dataset.push(candidates[i, :], loss_transformer(loss), variance)
